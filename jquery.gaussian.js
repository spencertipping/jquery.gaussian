...
(($ => {
  var gaussian_function  = (c, x) => Math.exp (-(x * x) / (2.0 * c * c));
  var linear_sequence    = (x0, d, n) => {var xs = []; for (var i = 0; i < n; ++i) xs.push (x0 + i * d); return xs};
  var centered_sequence  = (d, n) => linear_sequence (-d * (n - 1) / 2.0, d, n);
  var gaussian_sequence  = (c, n) => $.map (centered_sequence (c * 6.0 / n, n), x => gaussian_function (c, x));
  var normalized         = xs => {var total = 0; $.each (xs, (i, x) => {total += x}); return $.map (xs, x => x / total);};
  ...
  var do_gaussian_blur = (e, n, dx, dy) => {
    var position  = $(e).position();
    var opacity   = $(e).css('opacity');

    var positions = $.map (centered_sequence (1.0 / n, n), t => ({
      left: position.left + t * dx,
      top: position.top + t * dy
    }));

    var opacities = normalized (gaussian_sequence (3.0, n));
    var parent    = $(e).parent();
    var elements  = $();

    $.each (positions, (i, p) => {elements = elements.add ($(e).clone (false).attr ('id', '').addClass ('gaussian-clone').appendTo (parent).
  ...
  $.fn.gaussian = function (options) {
    return $(this).each (options === null ? unblur
                                          : (i, e) => {$(e).data('gaussian-clones') ? $(e).data('gaussian-clones').each (function () {$(this).gaussian (options)})
  ...
  $.fn.motionblur = function (options) {
    return $(this).each (function () {
      var p = $(this).position();
      var dx = options.left - p.left;
      var dy = options.top - p.top;
